...
// Source: public/src/js/controllers/tokens.js
angular.module('insight.tokens').controller('TokenRichlistController',
function($scope, $routeParams, $location, $interval, Global, Tokens, TokensTransactions, TokensRichlist, TokensAddressBalance, TokensAddressTransactions) {
  ...
  TokensRichlist.get({cctxid: $routeParams.cctxid},
    function(tokensRichlist) {
      console.warn('tokensRichlist', tokensRichlist);

      $scope.loading = false;
      $scope.richlist = tokensRichlist.addresses;
    },
    function(e) {
      var err = 'Could not get token richlist information' + e.toString();
      $scope.chart = {
        error: err
      };
    });
...
// Source: public/src/js/controllers/tokens.js
angular.module('insight.tokens').controller('TokenTradesController',
function($scope, $routeParams, $location, $interval, Global, Tokens, TokensTrades, TokensTransactions, TokensRichlist, TokensAddressBalance, TokensAddressTransactions) {
  var syncInterval;
  var pageNum = -1;
  var address = $routeParams.addrStr;
  ...
  TokensTrades.get({cctxid: $routeParams.cctxid},
  function(tokensTrades) {
    console.warn('tokensTrades', tokensTrades);

    $scope.loading = false;
    $scope.txs = tokensTrades.trades;
    $scope.stats = tokensTrades.stats;
  },
  function(e) {
    var err = 'Could not get token trades information' + e.toString();
    $scope.chart = {
      error: err
    };
  });
    ...
    return {
      ...
      link: function(scope, element, attr) {
        ...
        scope.keyup = function() {
          ...
          if (scope.explorerQuickFilter) {
            for (var i = 0; i < explorers.length; i++) {
              if ((!explorers[i].hasOwnProperty('ext') && explorers[i].substring(0, scope.explorerQuickFilter.length).toLowerCase() === scope.explorerQuickFilter.toLowerCase()) ||
                  (explorers[i].hasOwnProperty('ext') && explorers[i].name.substring(0, scope.explorerQuickFilter.length).toLowerCase() === scope.explorerQuickFilter.toLowerCase())) {
                items.push(explorers[i]);
              }
            }

            scope.explorers = items;
          } else {
            scope.explorers = explorers;
          }
        }
        ...
        $document.bind('click', function(event) {
          ...
          for (var i = 0; i < event.path.length; i++) {
            if (event.path[i].className &&
                event.path[i].className.indexOf('explorers--dropdown') > -1) {
              isParentNodeClicked = true;
            }
          }
